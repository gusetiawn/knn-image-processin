...
# Fungsi untuk ekstraksi fitur warna dan menyimpan gambar yang diubah serta histogram
def extract_color_features(image, processed_filename, histogram_filename):
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    hist = cv2.calcHist([hsv], [0, 1, 2], None, [8, 8, 8], [0, 256, 0, 256, 0, 256])
    hist = cv2.normalize(hist, hist).flatten()

    # Menyimpan gambar HSV yang diubah
    cv2.imwrite(processed_filename, hsv)

    # Membuat dan menyimpan gambar histogram
...
features = [extract_color_features(img, os.path.join(app.config['PROCESSED_FOLDER'], f'{i}.jpg'),
                                   os.path.join(app.config['HISTOGRAM_FOLDER'], f'{i}.png')) for i, img in enumerate(images)]
...
def predict():
    ...
    if file and allowed_file(file.filename):
        ...
        features = extract_color_features(image, processed_filename, histogram_filename)
...
# Route untuk mengakses gambar yang diproses
@app.route('/processed/<filename>')
def processed_file(filename):
    return send_from_directory(app.config['PROCESSED_FOLDER'], filename)

# Route untuk mengakses gambar histogram
